<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "GET" && strlen($_GET['id']) > 0) {
        # code...
        $selectUser = "SELECT * FROM users WHERE id = '{$_GET['id']}'";
...
<div id="userCU" class="max-w-50 w-100 min-h-50 bg-primary">
    <h1 class="h1 text-center fw-bolder text-white">User Edit</h1>
    <?php
        require_once('CUForm.php');
    ?>

    <div id="submitBox" class="d-flex justify-content-center m-3">
        <button type="button" id="btn" class="btn btn-success w-50 rounded-3 fw-bolder text-white" value="edit">Edit</button>
    </div>
            
            <div class="w-100">
                <button type="button" class="btn bg-white m-2 border-0">
                    <a href="list.php" class="text-primary fw-bolder text-decoration-none">Back</a>
                </button>
            </div>
